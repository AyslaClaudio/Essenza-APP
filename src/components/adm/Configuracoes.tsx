    ...
    <div className="space-y-4 animate-fadeIn">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {([
          ...
          <button key={t.id} onClick={() => setTab(t.id)} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap ${tab === t.id ? 'bg-[#B5652E] text-white' : 'bg-neutral-200 text-neutral-500'}`}>
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white border border-neutral-200 rounded-2xl p-5 space-y-4">
        <div className="flex items-center gap-2"><Store size={20} className="text-[#B5652E]" /><h3 className="text-neutral-900 font-semibold">Dados da Loja</h3></div>
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl p-5 space-y-4">
        <div className="flex items-center gap-2"><Clock size={20} className="text-[#B5652E]" /><h3 className="text-neutral-900 font-semibold">Horário</h3></div>
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl p-5 space-y-4">
        <div className="flex items-center gap-2"><Palette size={20} className="text-[#B5652E]" /><h3 className="text-neutral-900 font-semibold">Cores</h3></div>
        <div className="grid grid-cols-3 gap-3">
          <ColorField label="Primária" value={form.cor_primaria || '#B5652E'} onChange={(v) => set('cor_primaria', v)} />
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl p-5 space-y-4">
        <div className="flex items-center gap-2"><UserCog size={20} className="text-[#B5652E]" /><h3 className="text-neutral-900 font-semibold">Financeiro & Segurança</h3></div>
        ...
        <label className="flex items-center gap-2 text-neutral-700 text-sm">
          <input type="checkbox" checked={form.tabela_bloqueada ?? false} onChange={(e) => set('tabela_bloqueada', e.target.checked)} className="w-4 h-4 accent-[#B5652E]" />
          Bloquear edição da Tabela v12
        </label>
      </div>

      <button onClick={save} disabled={saving} className="w-full bg-[#B5652E] text-white py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 disabled:opacity-50">
  ...
  return (
    <div className="bg-white border border-neutral-200 rounded-2xl p-5 space-y-3">
      <div className="flex items-center gap-2">
        <Bluetooth size={20} className="text-[#B5652E]" />
      ...
      <div className="flex gap-2">
        {conectada ? (
          ...
          <button onClick={conectar} disabled={conectando || !bluetoothSuportado()} className="w-full flex items-center justify-center gap-2 bg-[#B5652E] text-white py-2.5 rounded-xl font-semibold text-sm disabled:opacity-50">
        ...
        {impressoras.map((imp) => (
          <div key={imp.id} className="bg-white border border-neutral-200 rounded-2xl p-4">
            <div className="flex items-start justify-between mb-2">
              <Printer size={24} className="text-[#B5652E]" />
    ...
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white border border-neutral-200 rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          <label className="flex items-center gap-2 text-neutral-700 text-sm">
            <input type="checkbox" checked={ativa} onChange={(e) => setAtiva(e.target.checked)} className="w-4 h-4 accent-[#B5652E]" /> Ativa
          </label>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white border border-neutral-200 rounded-2xl p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-2 mt-5">
          <button onClick={onClose} className="flex-1 py-3 bg-neutral-200 text-neutral-500 rounded-xl">Cancelar</button>
          <button onClick={save} disabled={!bairro} className="flex-1 py-3 bg-[#B5652E] text-white rounded-xl font-semibold disabled:opacity-50">Salvar</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {usuario?.role === 'gerente' && (
          <button onClick={() => setShowForm(true)} className="flex items-center gap-2 bg-[#B5652E] text-white px-4 py-2 rounded-xl text-sm font-semibold">
        ...
        {users.map((u) => (
          <div key={u.id} className="bg-white border border-neutral-200 rounded-xl p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-[#B5652E]/20 flex items-center justify-center text-[#B5652E] font-bold">
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white border border-neutral-200 rounded-2xl p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2 mt-5">
              ...
              <button onClick={createUser} className="flex-1 py-3 bg-[#B5652E] text-white rounded-xl font-semibold">Criar</button>
            </div>
          </div>
        </div>
      )}
...
function ConfigMarketing() {
  ...
  return (
    <div className="space-y-4">
      {/* Push notification */}
      <div className="bg-white border border-neutral-200 rounded-2xl p-5 space-y-3">
        <div className="flex items-center gap-2"><Bell size={20} className="text-[#B5652E]" /><h3 className="text-neutral-900 font-semibold">Notificação Push</h3></div>
        <textarea value={pushMsg} onChange={(e) => setPushMsg(e.target.value)} placeholder="Ex: Quarta da Pizza: 2 Grandes R$79,90" className="w-full bg-neutral-100 border border-neutral-200 rounded-xl px-4 py-3 text-neutral-900 min-h-[80px] focus:border-[#B5652E] focus:outline-none" />
        <button onClick={sendPush} disabled={!pushMsg} className="w-full bg-[#B5652E] text-white py-3 rounded-xl font-semibold disabled:opacity-50">
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl p-5 space-y-3">
        ...
        <label className="flex items-center gap-2 text-neutral-700 text-sm">
          ...
          }} className="w-4 h-4 accent-[#B5652E]" /> Ativar programa de fidelidade
        </label>
...
function Field({ label, value, onChange, type = 'text' }: { label: string; value: string; onChange: (v: string) => void; type?: string }) {
  return (
    <div>
      ...
      <input type={type} step={type === 'number' ? '0.01' : undefined} value={value} onChange={(e) => onChange(e.target.value)} className="w-full bg-neutral-100 border border-neutral-200 rounded-xl px-4 py-2.5 text-neutral-900 mt-1 focus:border-[#B5652E] focus:outline-none" />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mt-1">
        ...
        <input value={value} onChange={(e) => onChange(e.target.value)} className="flex-1 bg-neutral-100 border border-neutral-200 rounded-xl px-3 py-2.5 text-neutral-900 text-sm focus:border-[#B5652E] focus:outline-none" />
      </div>
    </div>
  );
}
