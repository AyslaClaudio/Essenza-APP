        ...
        <div className="flex gap-2">
          {(['ativos', 'hoje', 'todos'] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFiltro(f)}
              className={`px-4 py-2 rounded-xl text-sm font-medium capitalize ${filtro === f ? 'bg-[#F26522] text-white' : 'bg-neutral-200 text-neutral-500'}`}
  ...
  return (
    <div className="bg-white border border-neutral-200 rounded-2xl p-4 hover:border-neutral-600 transition-colors">
      ...
      <div className="flex gap-2">
        ...
        {nextStatus && (
          <button onClick={() => onStatus(nextStatus)} className="flex-1 py-2 bg-[#F26522] text-white rounded-xl text-sm font-medium hover:bg-[#f6121d]">
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white border border-neutral-200 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {nextStatus && (
          <button onClick={() => onStatus(nextStatus)} className="w-full py-3 bg-[#F26522] text-white rounded-xl font-bold mb-2">
