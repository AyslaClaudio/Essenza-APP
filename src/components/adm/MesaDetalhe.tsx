      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 bg-[#B91C1C] hover:bg-red-600 text-white text-sm px-4 py-2.5 rounded-xl font-bold transition-all active:scale-95"
          >
      ...
      >
        ...
        <div className="p-4 space-y-4">
          ...
          <input
            ...
            className="w-full bg-neutral-100 border border-neutral-200 rounded-xl px-3 py-2.5 text-sm text-neutral-900 focus:border-[#B91C1C] focus:outline-none"
          />
          ...
          <div className="flex gap-2 overflow-x-auto pb-0.5">
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors ${catFiltro === 'todas' ? 'bg-[#B91C1C] text-white' : 'bg-neutral-100 border border-neutral-200 text-neutral-500'}`}
            >Todos</button>
            {categorias.map((c) => (
              <button
                key={c}
                onClick={() => setCatFiltro(c)}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors ${catFiltro === c ? 'bg-[#B91C1C] text-white' : 'bg-neutral-100 border border-neutral-200 text-neutral-500'}`}
              >{c}</button>
            ))}
          </div>
            ...
            {filtrados.map((p) => (
              <button
                ...
                className={`flex items-center gap-3 px-2.5 py-2 rounded-lg border text-left text-sm transition-colors ${
                  prod1Id === p.id
                    ? 'border-[#B91C1C] bg-[#B91C1C]/10 text-[#B91C1C] font-semibold'
          ...
          <label className="flex items-center gap-2 text-sm text-neutral-700 cursor-pointer">
            <input type="checkbox" checked={meioMeio} onChange={(e) => setMeioMeio(e.target.checked)} className="accent-[#B91C1C]" />
        ...
        <div className="p-4 border-t border-neutral-200 sticky bottom-0 bg-white space-y-2">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 border border-[#B91C1C] text-[#B91C1C] rounded-xl py-2.5 font-bold hover:bg-[#B91C1C]/10 disabled:opacity-40 disabled:hover:bg-transparent"
          >
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 bg-[#B91C1C] hover:bg-red-600 text-white rounded-xl py-2.5 font-bold disabled:opacity-40"
            >
              ...
              {FORMAS_PAGAMENTO.map((f) => (
                <button
                  ...
                  className={`py-2.5 rounded-xl text-sm font-semibold border transition-colors ${
                    forma === f ? 'border-[#B91C1C] bg-[#B91C1C]/10 text-[#B91C1C] font-semibold' : 'border-neutral-200 text-neutral-700 hover:bg-neutral-100'
                  }`}
                >
                  {f}
                </button>
