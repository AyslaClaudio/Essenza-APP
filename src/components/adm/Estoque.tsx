  ...
  return (
    <div className="space-y-4 animate-fadeIn">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        <button onClick={() => setTab('ingredientes')} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap ${tab === 'ingredientes' ? 'bg-[#B91C1C] text-white' : 'bg-neutral-200 text-neutral-500'}`}>
          <Package size={16} /> Ingredientes
        </button>
        <button onClick={() => setTab('ficha')} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap ${tab === 'ficha' ? 'bg-[#B91C1C] text-white' : 'bg-neutral-200 text-neutral-500'}`}>
          <FlaskConical size={16} /> Ficha Técnica
        </button>
        <button onClick={() => setTab('previsao')} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap ${tab === 'previsao' ? 'bg-[#B91C1C] text-white' : 'bg-neutral-200 text-neutral-500'}`}>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" step="0.001" value={estoque} onChange={(e) => setEstoque(e.target.value)} className="w-full bg-neutral-100 border border-neutral-200 rounded-xl px-4 py-2.5 text-neutral-900 mt-1 focus:border-[#B91C1C] focus:outline-none" />
            </div>
            <div>
              <label className="text-neutral-500 text-sm">Estoque Mínimo</label>
              <input type="number" step="0.001" value={minimo} onChange={(e) => setMinimo(e.target.value)} className="w-full bg-neutral-100 border border-neutral-200 rounded-xl px-4 py-2.5 text-neutral-900 mt-1 focus:border-[#B91C1C] focus:outline-none" />
            </div>
          </div>
        </div>
          ...
          <div className="space-y-1 max-h-[400px] overflow-y-auto">
            {produtos.map((p) => (
              <button
                ...
                className={`w-full text-left p-3 rounded-xl flex items-center justify-between ${selectedProduto?.id === p.id ? 'bg-[#B91C1C] text-white' : 'bg-neutral-100 text-neutral-700 hover:bg-neutral-200'}`}
              >
                <div>
                  ...
                  return (
                    <div key={f.id} className="flex items-center justify-between bg-neutral-100 rounded-xl p-2.5">
                      <div className="flex items-center gap-2">
                        <Link2 size={14} className="text-[#B91C1C]" />
...
function AddIngredienteRow({ ingredientes, onAdd }: { ingredientes: Ingrediente[]; onAdd: (id: string, qtd: number) => void }) {
  ...
  return (
    <div className="flex gap-2">
      <select value={selId} onChange={(e) => setSelId(e.target.value)} className="flex-1 bg-neutral-100 border border-neutral-200 rounded-xl px-3 py-2 text-neutral-900 text-sm focus:border-[#B91C1C] focus:outline-none">
        <option value="">Ingrediente...</option>
        {ingredientes.map((i) => <option key={i.id} value={i.id}>{i.nome} ({brl(i.custo_por_unidade)}/{i.unidade})</option>)}
      </select>
      <input type="number" step="0.001" value={qtd} onChange={(e) => setQtd(e.target.value)} placeholder="Qtd" className="w-20 bg-neutral-100 border border-neutral-200 rounded-xl px-3 py-2 text-neutral-900 text-sm focus:border-[#B91C1C] focus:outline-none" />
      <button onClick={() => { if (selId && qtd) { onAdd(selId, parseFloat(qtd)); setSelId(''); setQtd(''); } }} className="px-3 py-2 bg-[#B91C1C] text-white rounded-xl">
        <Plus size={16} />
      </button>
    </div>
  );
