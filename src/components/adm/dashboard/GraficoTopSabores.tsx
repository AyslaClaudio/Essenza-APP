...
export function GraficoTopSabores({ data }: { data: SaborTop[] }) {
  ...
  return (
    <div className="bg-white border border-neutral-200 rounded-2xl p-5">
      <h3 className="text-neutral-900 font-bold mb-4">Top 5 Sabores</h3>
      {data.length === 0 ? (
          ...
          {data.map((s, i) => (
            <div key={s.nome} className="space-y-1">
              <div className="flex items-center justify-between text-sm">
                <span className="text-neutral-900 truncate flex items-center gap-2">
                  <span className="text-neutral-400 font-bold w-4">{i + 1}</span>
                  {s.nome}
                </span>
