      ...
      <div className="p-4 border-b border-neutral-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white">
        ...
        <div className="flex bg-neutral-100 p-1.5 rounded-xl border border-neutral-200 w-full sm:w-auto">
          <button
            ...
            className={`flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold transition-all ${tab === 'conversas' ? 'bg-[#F26522] text-white' : 'text-neutral-500 hover:text-neutral-900'}`}
          >
            <MessagesSquare size={14} /> Conversas
          </button>
          <button
            onClick={() => setTab('conhecimento')}
            className={`flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold transition-all ${tab === 'conhecimento' ? 'bg-[#F26522] text-white' : 'text-neutral-500 hover:text-neutral-900'}`}
          >
            ...
            conversas.map(conversa => {
              ...
              return (
                <button
                  ...
                  className={`w-full p-4 flex flex-col gap-1.5 text-left transition-colors ${active ? 'bg-neutral-200/80 border-l-4 border-l-[#F26522]' : 'hover:bg-neutral-100/55'}`}
                >
    ...
    <div className="flex-1 p-6 overflow-y-auto space-y-6 bg-neutral-50">
      ...
      <div className="bg-white border border-neutral-200 rounded-xl p-4 space-y-3 max-w-2xl">
        ...
        <textarea
          ...
          rows={3}
          className="w-full bg-neutral-100 border border-neutral-200 rounded-xl px-3 py-2 text-sm text-neutral-900 focus:border-[#F26522] focus:outline-none resize-none"
        />
        <button
          onClick={addItem}
          disabled={saving || !topico.trim() || !conteudo.trim()}
          className="flex items-center gap-2 bg-[#F26522] hover:bg-red-600 text-white text-sm px-4 py-2.5 rounded-xl font-bold transition-all active:scale-95 disabled:opacity-40"
        >
