    ...
    <div className="min-h-[80vh] flex flex-col bg-white rounded-2xl border border-neutral-200 overflow-hidden">
      ...
      <div className="p-4 border-b border-neutral-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white">
        ...
        <div className="flex bg-neutral-100 p-1.5 rounded-xl border border-neutral-200 w-full sm:w-auto">
          <button 
            ...
            className={`flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold transition-all ${activeTab === 'simulator' ? 'bg-[#B91C1C] text-white' : 'text-neutral-500 hover:text-neutral-900'}`}
          >
            <Smartphone size={14} /> Simulador
          </button>
          <button 
            onClick={() => setActiveTab('settings')} 
            className={`flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold transition-all ${activeTab === 'settings' ? 'bg-[#B91C1C] text-white' : 'text-neutral-500 hover:text-neutral-900'}`}
          >
            <Settings size={14} /> Configurações
          </button>
          <button 
            onClick={() => setActiveTab('real-connection')} 
            className={`flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold transition-all ${activeTab === 'real-connection' ? 'bg-[#B91C1C] text-white' : 'text-neutral-500 hover:text-neutral-900'}`}
          >
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden min-h-[500px]">
            ...
            <div className="w-full md:w-80 border-b md:border-b-0 md:border-r border-neutral-200 flex flex-col bg-neutral-50">
              <div className="p-3 border-b border-neutral-200 flex justify-between items-center">
                <span className="text-xs font-bold text-neutral-500 uppercase tracking-wider flex items-center gap-2">
                  Conversas Ativas
                  {dbLoading && <RefreshCw size={12} className="animate-spin text-[#B91C1C]" />}
                </span>
                <button 
                  onClick={createNewChat} 
                  className="bg-neutral-200 hover:bg-[#B91C1C] text-white text-xs px-2.5 py-1.5 rounded-lg border border-neutral-200 transition-colors font-medium"
                >
                  Novo Chat
                </button>
              </div>
                  ...
                  chats.map(chat => {
                    ...
                    return (
                      <button 
                        ...
                        className={`w-full p-4 flex flex-col gap-1 text-left transition-colors ${active ? 'bg-neutral-200/80 border-l-4 border-l-[#B91C1C]' : 'hover:bg-neutral-100/55'}`}
                      >
                ...
                <>
                  ...
                  <div className="p-3 border-t border-neutral-200 bg-neutral-50 flex gap-2">
                    <input 
                      ...
                      disabled={isTyping}
                      className="flex-1 bg-neutral-100 text-neutral-900 placeholder-neutral-500 border border-neutral-200 rounded-xl px-4 py-3 text-sm focus:border-[#B91C1C] focus:outline-none disabled:opacity-50"
                    />
                    <button 
                      onClick={handleSendMessage}
                      disabled={isTyping || !inputText.trim()}
                      className="w-12 h-12 bg-[#B91C1C] hover:bg-red-600 text-white rounded-xl flex items-center justify-center transition-colors disabled:opacity-40"
                    >
                      <Send size={18} />
                    </button>
                  </div>
        ...
        {activeTab === 'settings' && (
          <div className="flex-1 p-6 overflow-y-auto space-y-6 bg-neutral-50">
            <div className="flex items-center gap-2 text-neutral-900 font-bold text-base border-b border-neutral-200 pb-2">
              <Settings size={18} className="text-[#B91C1C]" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div className="bg-white border border-neutral-200 rounded-xl p-4 space-y-3">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      placeholder="AIzaSy..."
                      className="w-full bg-neutral-100 border border-neutral-200 rounded-xl px-3 py-2 text-sm text-neutral-900 focus:border-[#B91C1C] focus:outline-none"
                    />
                ...
                <div className="bg-white border border-neutral-200 rounded-xl p-4 space-y-3">
                  <div className="flex items-center gap-2 text-neutral-900 font-semibold text-sm">
                    <Smartphone size={16} className="text-[#B91C1C]" />
              ...
              <div className="flex flex-col h-full">
                <div className="bg-white border border-neutral-200 rounded-xl p-4 flex-1 flex flex-col gap-2 min-h-[300px]">
                  ...
                  <textarea 
                    ...
                    className="flex-1 bg-neutral-100 border border-neutral-200 rounded-xl p-3 text-xs text-neutral-700 focus:border-[#B91C1C] focus:outline-none font-mono resize-none leading-relaxed"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button 
                ...
                className="flex items-center gap-2 bg-[#B91C1C] hover:bg-red-600 text-white text-sm px-5 py-2.5 rounded-xl font-bold transition-all active:scale-95 shadow-lg shadow-red-950/40"
              >
        ...
        {activeTab === 'real-connection' && (
          <div className="flex-1 p-6 overflow-y-auto space-y-6 bg-neutral-50">
            <div className="flex items-center gap-2 text-neutral-900 font-bold text-base border-b border-neutral-200 pb-2">
              <Smartphone size={18} className="text-[#B91C1C]" />
            ...
            <div className="max-w-3xl space-y-4">
              ...
              <div className="bg-neutral-100 border border-neutral-200 rounded-xl p-5 space-y-4">
                <h4 className="font-bold text-neutral-900 text-sm flex items-center gap-2">
                  <span className="w-6 h-6 rounded-full bg-[#B91C1C] flex items-center justify-center text-xs text-white">1</span>
              ...
              <div className="bg-neutral-100 border border-neutral-200 rounded-xl p-5 space-y-3">
                <h4 className="font-bold text-neutral-900 text-sm flex items-center gap-2">
                  <span className="w-6 h-6 rounded-full bg-[#B91C1C] flex items-center justify-center text-xs text-white">2</span>
